"""Ligand-map preprocessing and tiled inference helpers."""
...
from emready.models.bimcunet_multitask import BiMCUnetMultiTask
...
def clip_exp_map(exp_map: np.ndarray, percentile: float) -> np.ndarray:
    ...
    exp_max = float(np.percentile(positive, percentile))
    if exp_max <= 0:
        return np.zeros_like(exp_map, dtype=np.float32)
    return np.clip(exp_map, 0.0, exp_max).astype(np.float32)


def normalize_exp_to_unit(exp_map: np.ndarray) -> np.ndarray:
    exp_max = float(np.max(exp_map))
    if exp_max <= 0:
        return np.zeros_like(exp_map, dtype=np.float32)
    return (exp_map / exp_max).astype(np.float32)


def zscore_map(map_data: np.ndarray, name: str = "exp_map") -> np.ndarray:
    mean = float(np.mean(map_data))
    std = float(np.std(map_data))
    print(f"# {name} mean/std before z-score: {mean:.6f} / {std:.6f}")
    if std < 1e-8:
        print(f"# Warning: {name} std is too small; returning zeros.")
        return np.zeros_like(map_data, dtype=np.float32)
    return ((map_data - mean) / std).astype(np.float32)


def preprocess_exp_map(exp_map: np.ndarray, percentile: float) -> np.ndarray:
    exp_map = clip_exp_map(exp_map, percentile)
    exp_map = normalize_exp_to_unit(exp_map)
    return zscore_map(exp_map, "exp_map")
...
def load_ligand_model(weight_file: Path, device: torch.device) -> BiMCUnetMultiTask:
    print(f"# Loading ligand model weights from {weight_file}")
    model = BiMCUnetMultiTask(**LIGAND_MODEL_KWARGS)
...
def infer_ligand_sim_class(
    ...
    *,
    ...
    sim_sum = np.zeros((depth, height, width), dtype=np.float32)
    ...
    def flush_batch() -> None:
        ...
        with torch.inference_mode():
            out = model(x)
            sim = out["sim"].squeeze(1).detach().cpu().numpy().astype(np.float32, copy=False)
    ...
    sim_avg = sim_sum / np.clip(weight_sum, a_min=1e-6, a_max=None)
    class_prob = torch.softmax(torch.from_numpy(logits_avg), dim=0).numpy().astype(np.float32, copy=False)
    class_idx = np.argmax(logits_avg, axis=0).astype(np.float32, copy=False)

    pred_ligand_sim = crop_to_original(sim_avg, original_shape, crop_start)
    pred_class = crop_to_original(class_idx, original_shape, crop_start)
    pred_class_prob = crop_to_original(class_prob, original_shape, crop_start)
    return pred_ligand_sim, pred_class, pred_class_prob


def build_ligand_mask(
    ...
    *,
    ...
    prob = class_prob[int(ligand_class_id)].astype(np.float32, copy=False)
    masked = ligand_sim * prob
    if class_threshold > 0.0:
        masked = np.where(prob >= float(class_threshold), masked, 0.0).astype(np.float32, copy=False)
    return masked


def build_ligand_binary_mask(class_idx: np.ndarray, ligand_class_id: int) -> np.ndarray:
    """Argmax class map -> binary mask (1 where class equals ligand_class_id)."""
    return np.where(class_idx == float(ligand_class_id), 1.0, 0.0).astype(np.float32, copy=False)
